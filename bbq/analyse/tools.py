...
def extrema(data: pd.DataFrame, *, field='close', cmp_func=np.greater) -> Tuple[List, List, List]:
    """
    求极值点
    ...
    x_trade_date = [dt.strftime('%Y-%m-%d') for dt in data['trade_date']]

    x_data_ext = signal.argrelextrema(x_data, cmp_func)
    y_data_ext = x_data[x_data_ext[0]]

    x_index = np.array(x_trade_date)[x_data_ext[0]]

    return list(x_index), list(x_data_ext[0]), list(y_data_ext)


def linear_fitting(data: pd.DataFrame, *, field='close', cmp_func=np.greater) -> \
    ...
    x_index, x_data_ext, y_data_ext = extrema(data=data, field=field, cmp_func=cmp_func)
...
def plot(df: pd.DataFrame, *, render_html=None, basic=None, overlap=None):
    pass
